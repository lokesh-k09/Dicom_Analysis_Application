...
import threading
import queue
import shutil                 # ← for cleaning up the QA folder
...
from process_utils import process_dat_file
import os
from pathlib import Path

# Force working directory to where this script is
os.chdir(Path(__file__).parent)
print(f"📂 Working directory set to: {Path.cwd()}")
msg_queue = queue.Queue()

def worker(input_dir, output_dir):
    # 1) gather .dat files
    files = [
...
frm1 = tk.Frame(root); frm1.pack(fill=tk.X, padx=5, pady=5)
tk.Button(frm1, text="Select input folder", command=select_input_folder).pack(side=tk.LEFT)
in_entry = tk.Entry(frm1, width=50); in_entry.pack(side=tk.LEFT, padx=5)

frm2 = tk.Frame(root); frm2.pack(fill=tk.X, padx=5, pady=5)
tk.Button(frm2, text="Select output folder", command=select_output_folder).pack(side=tk.LEFT)
out_entry = tk.Entry(frm2, width=50); out_entry.pack(side=tk.LEFT, padx=5)

frm3 = tk.Frame(root); frm3.pack(fill=tk.X, padx=5, pady=5)
run_btn = tk.Button(frm3, text="Run", command=on_run_clicked); run_btn.pack(side=tk.LEFT)
