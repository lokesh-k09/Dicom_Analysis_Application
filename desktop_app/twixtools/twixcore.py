...
from . import twixprot
from . import helpers
from . import mdb as mdb_module
from . import hdr_def
...
def read_twix(infile, include_scans=None, parse_prot=True, parse_data=True, parse_pmu=True,
    ...
    for s in range(NScans):
        ...
        while pos + 128 < scanEnd:  # fail-safe not to miss ACQEND
            fid.seek(pos, os.SEEK_SET)
            try:
                mdb = mdb_module.Mdb(fid, version_is_ve)
            except ValueError:
        ...
        if parse_geometry:
            if not parse_prot:
                ...
                out[-1]['geometry'] = geometry_module.Geometry.create_for_all_slices(out[-1])
